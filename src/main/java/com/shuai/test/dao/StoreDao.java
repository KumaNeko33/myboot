...
public interface StoreDao extends BaseDao<StoreRebateAudit,Long> {
    ...
    @Query(nativeQuery = true,value = "SUM(IFNULL(a.stock_in_point,0)+IFNULL(a.stock_out_point,0)) AS 'monthPoint',SUM(IFNULL(a.stock_in_rebate,0)+IFNULL(a.stock_out_rebate,0)) AS 'monthRebate',a.valid_in_quantity FROM t_store_rebate_audit a WHERE 1=1 AND a.store = :storeId AND a.year = :year AND a.quarter = :month")
